...
public class Level : MonoBehaviour
{
        ...
        for (int i = 0; i < slotNum; i++)
        {
            if (Random.Range(0, 10) < 4)    // Vũ khí
            {
                ...
                button[i].onClick.AddListener(delegate {
                    // CHÈN TẠI ĐÂY: Phát tiếng click phản hồi sau khi chọn vũ khí thành công
            ...
            else  // Phụ kiện (Accessory)
            {
                ...
                button[i].onClick.RemoveAllListeners();
                button[i].onClick.AddListener(delegate {
                    // CHÈN TẠI ĐÂY: Phát tiếng click phản hồi sau khi chọn phụ kiện thành công
                    PlayUpgradeSelectSound();
    ...
    // Hàm phụ trợ phát tiếng khi bấm chọn nút nâng cấp UI
    private void PlayUpgradeSelectSound()
    {
        if (AudioManager.GetInstance() != null)
        {
            AudioManager.GetInstance().PlaySFX(AudioManager.GetInstance().afterUpgradeSelectSFX);
        }
    }
